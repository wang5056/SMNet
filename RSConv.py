...
class RSConvRegressor(nn.Module):
    def __init__(self, input_nc, num_classes, output_dim):
        super(RSConvRegressor, self).__init__()
        self.rsconv = RSConv(architecture="unet", input_nc=input_nc, num_layers=4, output_nc=num_classes)
    ...
    def forward(self, tdata):
        rsconv_output = self.rsconv(tdata)
        x_reshaped = rsconv_output.x.permute(0, 2, 1)
...
# Input training data
model_save_path = "RSConv_regression_ionic_20000.pth"
csv_name = 'predictions_ionic_RSConv_testdata_20000.csv'
...
model = RSConvRegressor(input_nc, 1024, output_dim)
mse_loss = MSELoss()
mae_loss = L1Loss()
